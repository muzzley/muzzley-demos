...
var DEFAULT_WIDTH = 900,
	...
	function updatePlayer() {
		...
		// No energy – no game
		if( player.energy === 0 ) {
			$("#qrCode #start").text("Touch to start again.");
			stop();
		}
	...
	function onDocumentMouseDownHandler(){
		mouse.down = true;
	}
	
	function onDocumentMouseMoveHandler(e){
		mouse.previousX = mouse.x;
		mouse.previousY = mouse.y;
		
		//mouse.x = e.clientX- (window.innerWidth - world.width) * 0.5;
		//mouse.y = e.clientY - (window.innerHeight - world.height) * 0.5;
		mouse.x = e.clientX;
		mouse.y = e.clientY;
	...
	function onDocumentMouseUpHandler() {
		mouse.down = false;
	}
	...
	function onCanvasTouchMoveHandler() {
		if(event.touches.length == 1) {
			event.preventDefault();
